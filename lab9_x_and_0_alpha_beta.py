...
def alpha_beta(alpha, beta, stare):
    ...
    if stare.j_curent == Joc.JMAX:
        ...
        scor_curent = -9999  #echivalent: float(-int)
...
def main():
    ...
    x = int(input('Cu ce algoritm doriti sa jucati? (1 = MinMax, 2 = AlphaBeta) '))
    algoritm_valid = False
    if x != 1 and x != 2:
        while algoritm_valid == False:
            print("Nu ati ales o varianta valida. Selectati doar 1 sau 2. \n")
            x = int(input('Cu ce algoritm doriti sa jucati? (1 = MinMax, 2 = AlphaBeta) '))
            if x == 1 or x == 2:
                algoritm_valid = True
